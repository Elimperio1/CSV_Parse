...
def extract_transactions_vision(pdf_bytes, bank, stream_status=None):
    ...
    # Use vision-specific prompt if available, otherwise fall back to standard prompt
    ...
    if stream_status:
        stream_status.caption(f"Response complete — {input_tokens} input / {output_tokens} output tokens. Parsing...")
...
def _call_claude_stream(pdf_b64, prompt, stream_status, chunk_label=""):
    """Stream a PDF to Claude. Returns (raw_text, input_tokens, output_tokens)."""
    client = get_client()
    raw = ""
    chunk_count = 0
    with client.messages.stream(
        ...
        # Get actual token counts from Anthropic (not chunk counts)
        final_msg = stream.get_final_message()
        input_tokens  = final_msg.usage.input_tokens
        output_tokens = final_msg.usage.output_tokens
...
def extract_transactions(pdf_bytes, bank, stream_status=None):
    """Extract transactions from a text-layer PDF. Returns (rows, input_tokens, output_tokens)."""
    client = get_client()
    if not client:
        raise ValueError("No API key configured")
    prompt = PROMPTS[bank]
    ...
    if page_count <= CHUNK_SIZE:
        ...
        if stream_status:
            stream_status.caption(f"All {len(chunks)} chunks done — {total_input} input / {total_output} output tokens. Merging...")
...
def fmt_cost(usd, zar):
    return f"${usd:.4f} USD  ·  R{zar:.4f} ZAR"
...
defaults = {
    ...
    'history': [],
    'cached_upload_bytes': {},
    'uploader_key': 0,          # incremented to reset file uploader
...
if not check_api_configured():
    st.error(
        ...
        "Ask your administrator to add `ANTHROPIC_API_KEY` to the Streamlit app secrets."
    )
    st.stop()
...
if st.session_state.all_rows:
    ...
    with col5:
        st.markdown(
            ...
            f'<div style="font-size:10px;color:#2a4a2a;margin-top:2px;">${s_usd:.4f} · R{USD_ZAR_RATE}/$ avg</div>'
            f'</div>',
            unsafe_allow_html=True
        )
    st.markdown("")
...
uploaded_files = st.file_uploader(
    ...
    key=f"uploader_{st.session_state.uploader_key}"   # key change clears the widget
)
            ...
            if scanned:
                ...
                try:
                    raw, inp_tok, out_tok = extract_transactions_vision(
                        file_data['bytes'], confirmed_bank, stream_status=stream_status
                    )
                    vision_used = True
...
# ── Step 1: Confirmation panel ────────────────────────────────────────────────
    ...
    if new_files:
        ...
        # Cost warning — shown always so users understand API costs are non-recoverable
        st.info(
            "⚠️ **API cost notice:** Once processing begins, Anthropic charges per token processed. "
            "These costs cannot be reversed or refunded if you close the tab or navigate away mid-process. "
            f"Estimated cost: ~$0.01–$0.05 per file depending on statement length."
        )
        ...
        with col_confirm:
            if st.button(f"Confirm — process files as {selected_bank}", use_container_width=True):
        ...
        with col_cancel:
            if st.button("✗ Cancel", use_container_width=True):
                # Clear all upload state and reset the file uploader widget
                st.session_state.confirmed_bank      = None
                st.session_state.confirmed_files     = []
                st.session_state.cached_upload_bytes = {}
                st.session_state.uploader_key       += 1   # forces widget reset
                st.rerun()
        ...
        for idx, f in enumerate(st.session_state.processed_files):
            ...
            with col_a:
                ...
                if f['status'] == 'done':
                    ...
                    cost_tag    = f"  ·  ${f.get('cost_usd', 0):.4f} / R{f.get('cost_zar', 0):.4f}" if f.get('cost_usd') else ""
                    st.success(
                        f"**{f['name']}** [{bank_label}]{vision_tag} — "
    ...
    if not st.session_state.history:
        ...
        for hi, entry in enumerate(st.session_state.history):
            ...
            for fi, f in enumerate(entry['files']):
                ...
                col_a, col_b = st.columns([3, 1])
                with col_a:
                    st.markdown(f"&nbsp;&nbsp;&nbsp;{f['name']} — {f['txn_count']} transactions{fee_info}{cost_tag}")
                with col_b:
