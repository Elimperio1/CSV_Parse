...
# ─── PAGE CONFIG ─────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="El Imperio · Bank Statement Converter",
    page_icon=None,
    layout="wide",
...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&family=EB+Garamond:ital,wght@0,400;0,500;1,400&family=Montserrat:wght@300;400;500;600&display=swap');
...
div[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0d2157 0%, #0a1a45 100%) !important;
...
.ei-stat {
    ...
    text-align: center;
    box-shadow: 0 2px 8px rgba(13,33,87,0.06);
    transition: box-shadow 0.2s;
}
.ei-stat:hover { box-shadow: 0 4px 16px rgba(13,33,87,0.12); }
.ei-stat-number {
    font-family: 'Playfair Display', serif;
    font-size: 32px;
    color: #0d2157;
...
.ei-confirm-card {
    ...
    gap: 14px;
    align-items: center;
    box-shadow: 0 1px 4px rgba(13,33,87,0.06);
}
.ei-confirm-badge {
    background: #e8eef7;
    color: #0d2157;
...
.ei-confirm-name { font-size: 13px; color: #1a1a2e; font-weight: 500; }
.ei-confirm-note { font-size: 11px; color: #8a9bb5; margin-top: 2px; letter-spacing: 0.3px; }
...
.ei-empty {
    text-align: center;
    padding: 60px 40px;
    border: 2px dashed #d0cbbf;
    border-radius: 8px;
    margin-top: 20px;
    background: #faf9f5;
}
.ei-empty-icon { font-size: 40px; margin-bottom: 12px; opacity: 0.3; }
.ei-empty-title { font-family: 'Playfair Display', serif; font-size: 18px; color: #8a9bb5; margin-bottom: 6px; }
.ei-empty-sub { font-size: 11px; color: #b0a898; letter-spacing: 1px; text-transform: uppercase; }
...
.stProgress > div > div > div {
    background: linear-gradient(90deg, #0d2157, #1a3a7a) !important;
}
</style>
    ...
    with client.messages.stream(
        ...
        for text in stream.text_stream:
            ...
            if stream_status and token_count % 50 == 0:
                stream_status.markdown(f'<div style="color:#8a9bb5;font-size:12px;font-family:Montserrat,sans-serif;letter-spacing:1px">⟳ Vision mode — {token_count} tokens received...</div>', unsafe_allow_html=True)
    if stream_status:
        stream_status.markdown(f'<div style="color:#2e7d32;font-size:12px;font-family:Montserrat,sans-serif;letter-spacing:1px">✓ Complete — {token_count} tokens. Parsing...</div>', unsafe_allow_html=True)
...
def _call_claude_stream(pdf_b64, prompt, stream_status, chunk_label=""):
    ...
    with client.messages.stream(
        ...
        for text in stream.text_stream:
            ...
            token_count += 1
            if stream_status and token_count % 50 == 0:
                stream_status.markdown(f'<div style="color:#8a9bb5;font-size:12px;font-family:Montserrat,sans-serif;letter-spacing:1px">⟳ Processing{chunk_label} — {token_count} tokens received...</div>', unsafe_allow_html=True)
...
def extract_transactions(pdf_bytes, bank, stream_status=None):
    ...
    if page_count <= CHUNK_SIZE:
        ...
        if stream_status:
            stream_status.markdown(f'<div style="color:#2e7d32;font-size:12px;font-family:Montserrat,sans-serif;letter-spacing:1px">✓ All {len(chunks)} chunks complete — {total_tokens} total tokens.</div>', unsafe_allow_html=True)
        return all_rows
...
with st.sidebar:
    ...
    if selected_bank == "Capitec":
        st.markdown('<hr style="border-color:rgba(201,168,76,0.15);margin:16px 0">', unsafe_allow_html=True)
        st.markdown('<p style="font-size:9px;letter-spacing:3px;text-transform:uppercase;color:rgba(201,168,76,0.7);margin-bottom:6px">Capitec Fees</p>', unsafe_allow_html=True)
...
with _hcol2:
    ...
    <div style="font-family:\'Playfair Display\',serif;font-size:18px;color:#0d2157;letter-spacing:3px;text-transform:uppercase;font-weight:700">Bank Statement Converter</div>
    <div style="font-family:Montserrat,sans-serif;font-size:11px;color:#4a5568;letter-spacing:2px;text-transform:uppercase;margin-top:5px">Capitec · Investec · FNB · ABSA · Nedbank · Standard Bank · Powered by Claude AI</div>
    </div>''', unsafe_allow_html=True)
with _hcol3:
    st.markdown('<div style="display:flex;align-items:center;height:100%;padding:10px 0"><span style="background:rgba(13,33,87,0.08);border:1.5px solid #0d2157;border-radius:4px;padding:6px 14px;font-size:10px;color:#0d2157;letter-spacing:2px;text-transform:uppercase;font-family:Montserrat,sans-serif;white-space:nowrap;font-weight:600">PDF → CSV</span></div>', unsafe_allow_html=True)
st.markdown('<div style="height:3px;background:linear-gradient(90deg,#c9a84c,#a07a2a,#c9a84c);margin-bottom:24px"></div>', unsafe_allow_html=True)

# ─── API CHECK ───────────────────────────────────────────────────────────────
if not check_api_configured():
    st.error("**API key not configured.** Add `ANTHROPIC_API_KEY` to Streamlit app secrets. Go to: App Settings → Secrets.")
    st.stop()
...
if st.session_state.all_rows:
    ...
    with col1:
        st.markdown(f'<div class="ei-stat"><div class="ei-stat-number">{files_done}</div><div class="ei-stat-label">Files Processed</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown(f'<div class="ei-stat"><div class="ei-stat-number">{txn_count}</div><div class="ei-stat-label">Transactions</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown(f'<div class="ei-stat"><div class="ei-stat-number">{fee_count}</div><div class="ei-stat-label">Fee Rows</div></div>', unsafe_allow_html=True)
    with col4:
        st.markdown(f'<div class="ei-stat"><div class="ei-stat-number">{total}</div><div class="ei-stat-label">Total Rows</div></div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)

# ─── UPLOAD ───────────────────────────────────────────────────────────────────
st.markdown(f'<div class="ei-section-title">Upload {selected_bank} Statements</div>', unsafe_allow_html=True)
...
if st.session_state.confirmed_bank and st.session_state.confirmed_files:
    ...
    st.markdown(f'<div class="ei-section-title">Processing {total_files} file{"s" if total_files > 1 else ""} — {confirmed_bank}</div>', unsafe_allow_html=True)
    ...
    for i, file_data in enumerate(files_to_process):
        ...
        status.markdown(f'<div style="font-size:13px;color:#0d2157;font-family:Montserrat,sans-serif;margin:8px 0">Processing <strong>{file_data["name"]}</strong> ({i+1}/{total_files})</div>', unsafe_allow_html=True)
        stream_status.markdown('<div style="color:#8a9bb5;font-size:12px;font-family:Montserrat,sans-serif;letter-spacing:1px">⟳ Sending PDF to Claude...</div>', unsafe_allow_html=True)
        timing.caption(eta_label)

        try:
            scanned = is_scanned_pdf(file_data['bytes'])
            if scanned:
                status.markdown(f'<div style="font-size:13px;color:#0d2157;font-family:Montserrat,sans-serif;margin:8px 0">Processing <strong>{file_data["name"]}</strong> ({i+1}/{total_files}) — scanned PDF, using vision mode...</div>', unsafe_allow_html=True)
                try:
            ...
            stream_status.markdown(f'<div style="color:#2e7d32;font-size:12px;font-family:Montserrat,sans-serif;letter-spacing:1px">✓ Extracted {txn_rows} transactions in {elapsed_file}s</div>', unsafe_allow_html=True)
            ...
            if error_msg.startswith("VISION_FAILED"):
                ...
                st.error(f"**{file_data['name']}** — vision extraction failed. Scan quality may be too low.\n\nDetail: {detail}")
            st.session_state.processed_files.append({
                'name': file_data['name'], 'bank': confirmed_bank,
                'rows': [], 'status': 'error', 'error': error_msg
            })
    ...
    timing.caption(f"Completed in {elapsed_str}")
...
# ── CONFIRM PANEL ──────────────────────────────────────────────────────────────
    ...
    if new_files:
        st.markdown('<div class="ei-section-title">Confirm Extraction</div>', unsafe_allow_html=True)
        ...
        for f in new_files:
            ...
            file_rows.append((icon, f.name, note))

        for icon, fname, note in file_rows:
            icon_color = "#c9a84c" if icon == "⚠" else "#2e7d32"
            st.markdown(f'''
            <div class="ei-confirm-card">
                <div class="ei-confirm-badge" style="background:{"#fff8e8" if icon == "⚠" else "#e8f5e9"};color:{icon_color}">{icon}</div>
                <div>
                    <div class="ei-confirm-name">{fname}</div>
                    <div class="ei-confirm-note">{note}</div>
                </div>
            </div>''', unsafe_allow_html=True)

        if any_mismatch:
            st.markdown(f'<div class="ei-warn">⚠ One or more files may not match the selected bank. Processing with the wrong prompt wastes API tokens and produces incorrect results. Please verify your selection before proceeding.</div>', unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        col_confirm, col_cancel = st.columns(2)
        with col_confirm:
            if st.button(f"Confirm — extract as {selected_bank}", use_container_width=True):
...
with tab_results:
    if st.session_state.processed_files:
        ...
        with col_hdr:
            st.markdown('<div class="ei-section-title">Processed Files</div>', unsafe_allow_html=True)
        with col_clr:
            st.markdown("<br>", unsafe_allow_html=True)
        ...
        for idx, f in enumerate(st.session_state.processed_files):
            ...
            with col_a:
                ...
                if f['status'] == 'done':
                    ...
                    vision_tag = " · vision mode" if f.get("vision") else ""
                    elapsed_tag = f" · {f['elapsed']}s" if f.get("elapsed") else ""
                    st.markdown(f'<div class="ei-result-ok"><strong>{f["name"]}</strong> <span style="opacity:0.6">[{bank_label}]</span>{vision_tag} — {f["txn_count"]} transactions{fee_info} = {len(f["rows"])} total rows{elapsed_tag}</div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="ei-result-err"><strong>{f["name"]}</strong> [{bank_label}] — {f.get("error", "Unknown error")}</div>', unsafe_allow_html=True)
            with col_b:
                if f['status'] == 'done':
        ...
        if st.session_state.all_rows:
            ...
            st.markdown('<div class="ei-section-title">Download</div>', unsafe_allow_html=True)
            ...
            with col1:
                ...
                st.download_button(
                    ...
                    data=all_csv,
                    file_name="elimperio_all_transactions.csv",
                    mime='text/csv',
            ...
            with col2:
                ...
                if selected_month != 'All months':
                    ...
                    st.download_button(
                        ...
                        data=month_csv,
                        file_name=f"elimperio_{selected_month}.csv",
                        mime='text/csv',
                        use_container_width=True
                    )

            st.markdown('<hr class="ei-divider">', unsafe_allow_html=True)
            st.markdown('<div class="ei-section-title">Preview</div>', unsafe_allow_html=True)
        ...
        <div class="ei-empty">
            ...
            <div class="ei-empty-title">No statements processed yet</div>
            <div class="ei-empty-sub">Select your bank in the sidebar, then upload PDF statements</div>
            <div style="margin-top:16px;font-size:10px;color:#b0a898;letter-spacing:1px">{banks_str}</div>
            <div style="margin-top:8px;font-size:10px;color:#b0a898">Output: Date · Details · Amount · Pastel-ready</div>
        </div>''', unsafe_allow_html=True)
...
with tab_history:
    if not st.session_state.history:
        ...
        <div class="ei-empty">
            <div class="ei-empty-icon">⌛</div>
            <div class="ei-empty-title">No history yet</div>
            <div class="ei-empty-sub">Completed sessions will appear here · Last 3 sessions saved automatically</div>
        </div>''', unsafe_allow_html=True)
    else:
        for hi, entry in enumerate(st.session_state.history):
            st.markdown(f'<div style="font-family:Playfair Display,serif;font-size:16px;color:#0d2157;margin-bottom:8px"><strong>{entry["timestamp"]}</strong> — {entry["bank"]} — {len(entry["files"])} file{"s" if len(entry["files"]) > 1 else ""}</div>', unsafe_allow_html=True)
